...
// this crashes on Wine, see https://bugs.winehq.org/show_bug.cgi?id=59226
// also it's honestly not great UX, keeping the original name is probs a better option
...
std::wstring GetAutoGeneratedName(std::wstring original) {
    std::wstring aviPath = TMAudio::GetAviFilePath();
    if (aviPath.empty()) {
        aviPath = L"video.avi";
    }

    std::filesystem::path dir = std::filesystem::path(original).parent_path();
    std::filesystem::path filename = std::filesystem::path(aviPath).filename();

    std::filesystem::path path = (dir / filename).replace_extension(L".mp4");
    ...
    if (std::filesystem::exists(path, ec) && !ec) {
        ...
        for (int i = 2; i <= 9999; ++i) {
            std::filesystem::path candidate = dir / (stem + L"_" + std::to_wstring(i) + ext);
    ...
    if (ec) {
        MessageBoxW(nullptr, L"Failed to check if file exists for auto-generated name, using video.mp4 in current working directory.", L"VfW FFmpeg Bridge", MB_OK | MB_ICONERROR);
        return L"video.mp4";
    }
...
bool CodecState::SetRenderPath() {
    auto aviPath = TMAudio::GetAviFilePath();

    if (this->locationSelection == LocationSelection::Ask) {
        std::wstring base = this->path.empty() ? aviPath : this->path;

        auto savePath = Bridge::SaveDialog(GetAutoGeneratedName(base), L"MP4 Video (*.mp4)\0*.mp4\0MKV Video (*.mkv)\0*.mkv\0MOV Video (*.mov)\0*.mov\0All Files (*.*)\0*.*\0"s);

        if (!savePath.has_value()) {
            return false;
        }

        this->path = savePath.value();
        return true;
    } else if (this->locationSelection == LocationSelection::NextToAvi) {
        this->path = GetAutoGeneratedName(aviPath);
        return true;
    } else if (this->locationSelection == LocationSelection::Folder) {
        std::filesystem::path filename = std::filesystem::path(aviPath).filename();
        this->path = GetAutoGeneratedName((std::filesystem::path(this->otherLocation) / filename).wstring());
        return true;
    }
